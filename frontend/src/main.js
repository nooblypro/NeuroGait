...
// Configure Assessment Webpage URLs dynamically from VITE_ASSESSMENT_URL or environment
function setupCtaLinks() {
  const assessmentUrl = (import.meta.env.VITE_ASSESSMENT_URL || '').rstrip ? (import.meta.env.VITE_ASSESSMENT_URL || '').rstrip('/') : (import.meta.env.VITE_ASSESSMENT_URL || '').replace(/\/+$/, '');
  const recordedBtn = document.getElementById('cta-recorded');
  const liveBtn = document.getElementById('cta-live');

  const recordedPath = '/?view=assessment&mode=recorded';
  const livePath = '/?view=assessment&mode=live_demo';

  if (recordedBtn) {
    recordedBtn.href = assessmentUrl ? `${assessmentUrl}${recordedPath}` : recordedPath;
  }
  if (liveBtn) {
    liveBtn.href = assessmentUrl ? `${assessmentUrl}${livePath}` : livePath;
  }
}
